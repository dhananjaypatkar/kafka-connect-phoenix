...
public class PhoenixRecordParser implements EventParser {
  ...
  public Map<String, Object> parse(final String topic, final Schema schema, final Object value,
      final boolean isKey)
    ...
    try {
      ...
      keyValues = OBJECT_MAPPER
          .convertValue(valueNode.get("payload"), new TypeReference<Map<String, Object>>() {
          });
      if (keyValues == null) {
      ...
      final String errorMsg = String
          .format("Failed to parse the schema [%s] , value [%s] with ex [%s]", schema,
              value, ex.getMessage());
